export const AI_PROVIDERS = [
  ...
  {
    // Kimi (Moonshot AI) — hosted, OpenAI-compatible. Get a key at
    // platform.moonshot.ai. Models e.g. "kimi-k2-0711-preview",
    // "moonshot-v1-8k". Use "api.moonshot.cn" instead of "api.moonshot.ai"
    // for the China endpoint.
    id: "kimi",
    curl: `curl https://api.moonshot.ai/v1/chat/completions \\
  -H "Content-Type: application/json" \\
  -H "Authorization: Bearer {{API_KEY}}" \\
  -d '{
    "model": "{{MODEL}}",
  ...
  {
    // Generic local OpenAI-compatible server: LM Studio (default :1234),
    // llama.cpp server, vLLM, LocalAI, or self-hosted Kimi K2 open weights.
    // No API key required — remove the header entirely so nothing is demanded.
    // Change the host/port to match your server.
    id: "local-openai",
