...
// Native microphone capture is macOS-only: it exists specifically to avoid the
// WebView `getUserMedia` audio-session freeze that only affects WKWebView on
// macOS (see macos::MicInput). Windows/Linux keep the WebView mic path.
#[cfg(target_os = "macos")]
pub use macos::MicInput;
...
// ChannelPulse speaker input and stream
