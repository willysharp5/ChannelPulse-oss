...
struct SecureStorage {
    ...
    selected_channelpulse_model: Option<String>,
}
...
pub async fn secure_storage_save(app: AppHandle, items: Vec<StorageItem>) -> Result<(), String> {
    ...
    for item in items {
        match item.key.as_str() {
            "channelpulse_license_key" => storage.license_key = Some(item.value),
            "channelpulse_instance_id" => storage.instance_id = Some(item.value),
            "selected_channelpulse_model" => {
                storage.selected_channelpulse_model = Some(item.value)
            }
...
pub async fn secure_storage_get(app: AppHandle) -> Result<StorageResult, String> {
    ...
    if !storage_path.exists() {
        return Ok(StorageResult {
            license_key: None,
            instance_id: None,
            selected_channelpulse_model: None,
        });
    }
    ...
    Ok(StorageResult {
        ...
        selected_channelpulse_model: storage.selected_channelpulse_model,
    })
}
...
pub async fn secure_storage_remove(app: AppHandle, keys: Vec<String>) -> Result<(), String> {
    ...
    for key in keys {
        match key.as_str() {
            "channelpulse_license_key" => storage.license_key = None,
            "channelpulse_instance_id" => storage.instance_id = None,
            "selected_channelpulse_model" => storage.selected_channelpulse_model = None,
...
/// Returns a stable per-device identifier used to key the server-side trial.
#[tauri::command]
pub fn get_machine_id(app: AppHandle) -> Result<String, String> {
    app.machine_uid()
        .get_machine_uid()
        .map_err(|e| format!("Failed to get machine id: {}", e))?
        .id
        .ok_or_else(|| "No machine id available".to_string())
}
