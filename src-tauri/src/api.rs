...
// ChannelPulse Prompts API
#[derive(Debug, Serialize, Deserialize, Clone)]
pub struct ChannelPulsePrompt {
    title: String,
    prompt: String,
...
#[derive(Debug, Serialize, Deserialize)]
pub struct ChannelPulsePromptsResponse {
    prompts: Vec<ChannelPulsePrompt>,
    total: i32,
...
// Fetch ChannelPulse Prompts API
#[tauri::command]
pub async fn fetch_prompts() -> Result<ChannelPulsePromptsResponse, String> {
    let app_endpoint = get_app_endpoint()?;
    let api_access_key = get_api_access_key()?;
    ...
    let prompts_response: ChannelPulsePromptsResponse = response
        .json()
        .await
