...
/// Toggle OS content protection for the calling window. Content protection is
/// gated behind the user-facing "Privacy Mode" setting (default off) — see
/// `togglePrivacyMode()` in `app.context.tsx`. Also used by the bug-report
/// flow for a short manual-screenshot window.
#[tauri::command]
...
/// Capture the current window for a bug report: briefly disable content
/// protection, snapshot + crop to the window, then restore it to
/// `restore_protected` (the caller's current Privacy Mode state) — NOT
/// unconditionally `true`, since Privacy Mode defaults to off.
#[tauri::command]
pub async fn capture_bug_report_screenshot(
    window: tauri::WebviewWindow,
    restore_protected: bool,
) -> Result<String, String> {
    window
        .set_content_protected(false)
        .map_err(|e| format!("Failed to disable content protection: {}", e))?;

    // OS needs a short beat before the window is capturable.
    tokio::time::sleep(Duration::from_millis(200)).await;

    let result = capture_window_region_base64(&window).await;

    if let Err(e) = window.set_content_protected(restore_protected) {
        eprintln!("Failed to restore content protection after bug capture: {}", e);
    }

    result
}

async fn capture_window_region_base64(window: &tauri::WebviewWindow) -> Result<String, String> {
    let position = window
        .outer_position()
    ...
    let window_width = size.width.min(i32::MAX as u32) as i32;
    let window_height = size.height.min(i32::MAX as u32) as i32;
    let window_right = window_left.saturating_add(window_width);
    let window_bottom = window_top.saturating_add(window_height);
    let window_center_x = window_left.saturating_add(window_width / 2);
    let window_center_y = window_top.saturating_add(window_height / 2);
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let monitor = monitors
            .into_iter()
            .enumerate()
            .find_map(|(idx, monitor)| (idx == target_idx).then_some(monitor))
            .ok_or_else(|| "Failed to determine target monitor".to_string())?;
        ...
        let crop_x = (window_left - mon_x).max(0) as u32;
        let crop_y = (window_top - mon_y).max(0) as u32;
        let crop_w = (window_width.max(1) as u32).min(img_w.saturating_sub(crop_x));
        let crop_h = (window_height.max(1) as u32).min(img_h.saturating_sub(crop_y));

        if crop_w == 0 || crop_h == 0 {
            return Err("Window is outside the captured monitor".to_string());
        }

        let cropped = image.view(crop_x, crop_y, crop_w, crop_h).to_image();
        let mut png_buffer = Vec::new();
        PngEncoder::new(&mut png_buffer)
            .write_image(
                cropped.as_raw(),
